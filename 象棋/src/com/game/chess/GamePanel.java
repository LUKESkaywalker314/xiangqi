 ...
 * 功能：游戏面板<br>
 */
public class GamePanel extends JPanel implements MouseListener,MouseMotionListener,ActionListener, Serializable
{
	...
	private JComboBox<String> jcb_difficulty;
	...
	private void loadImage()
	{
		...
		{
			//棋盘图片
			this.imageIconChessBoard = new ImageIcon(new ImageIcon(this.getClass().getResource("/chess/chessBoard.png")).getImage().getScaledInstance(this.panelChess.getWidth(),this.panelChess.getHeight(),Image.SCALE_SMOOTH));	//缩放图片来适应标签大小
			//棋子图片
			for(int i=0;i<this.imageIconChess.length;i++)
			{
				this.imageIconChess[i] = new ImageIcon(new ImageIcon(this.getClass().getResource("/chess/chess"+i+".png")).getImage().getScaledInstance(this.chessSize,this.chessSize,Image.SCALE_SMOOTH));	//缩放图片来适应标签大小
			}
			//Logo图片
			this.imageIconLogo = new ImageIcon(new ImageIcon(this.getClass().getResource("/chess/logo.png")).getImage().getScaledInstance(100,50,Image.SCALE_SMOOTH));	//缩放图片来适应标签大小
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	private void initGame()
	{
		...
		// 设置搜索深度
		if ("高级".equals(this.jcb_difficulty.getSelectedItem().toString())) {
			this.gameLogic.setMaxDepth(2);
		} else {
			this.gameLogic.setMaxDepth(1);
		}
	...
	private void option()
	{
		...
		// 难度选择
		JLabel jlb_difficulty = new JLabel("难度选择：");
		jlb_difficulty.setBounds(this.panelChess.getWidth() + 22, 225, 100, 24);
		jlb_difficulty.setFont(new Font("微软雅黑", Font.PLAIN, 12));
		this.add(jlb_difficulty);
		this.jcb_difficulty = new JComboBox<String>(new String[]{"初级", "高级"});
		this.jcb_difficulty.setBackground(Color.WHITE);
		this.jcb_difficulty.setFont(new Font("微软雅黑", Font.PLAIN, 12));
		this.jcb_difficulty.setBounds(this.panelChess.getWidth() + 22, 250, 100, 24);
		this.jcb_difficulty.addActionListener(this);
		this.jcb_difficulty.setActionCommand("difficulty");
		this.add(this.jcb_difficulty);
		// 按钮
		this.jb_new = new JButton("开始游戏");
		this.jb_new.setFont(new Font("微软雅黑", Font.PLAIN, 12));
		this.jb_new.setBounds(this.panelChess.getWidth() + 22, 280, 100, 30);
		...
		this.jb_undo.setBounds(this.panelChess.getWidth() + 22, 315, 100, 30);
		...
		this.jb_surrender.setBounds(this.panelChess.getWidth() + 22, 350, 100, 30);
		...
		groupBoxRed.setBounds(this.panelChess.getWidth() + 22, 390, 100, 80);
		...
		groupBoxBlack.setBounds(this.panelChess.getWidth() + 22, 475, 100, 80);
	...
	public void newGame()
	{
		if (this.isGameOver) {
			...
			int result = JOptionPane.showConfirmDialog(null, "当前游戏尚未结束，确定要开始新游戏吗？", "确认", JOptionPane.YES_NO_OPTION);
	...
	public void setDifficulty(String difficulty) {
		this.jcb_difficulty.setSelectedItem(difficulty);
		this.initGame();
	}
	...
	public void saveGame() {
		try {
			File saveDir = new File("src/save");
			if (!saveDir.exists()) {
				saveDir.mkdirs();
			}
			File saveFile = new File(saveDir, "savegame.dat");
			ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(saveFile));
			oos.writeObject(this.mapChess);
			oos.writeObject(this.chessBoradState);
			oos.writeObject(this.listChess);
			oos.writeObject(this.redUndoNum);
			oos.writeObject(this.blackUndoNum);
			oos.writeObject(this.isFirstClick);
			oos.writeObject(this.firstClickChess);
			oos.writeObject(this.mapPointerChess);
			oos.writeObject(this.mapPointerMove);
			oos.writeObject(this.isGameOver);
			oos.close();
			JOptionPane.showMessageDialog(null, "游戏进度已保存！", "提示", JOptionPane.INFORMATION_MESSAGE);
		} catch (IOException e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "保存游戏进度时出现错误！", "错误", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void loadGame() {
		try {
			File saveFile = new File("src/save/savegame.dat");
			if (saveFile.exists() && saveFile.length() > 0) {
				ObjectInputStream ois = new ObjectInputStream(new FileInputStream(saveFile));
				this.mapChess = (Map<String, String>[]) ois.readObject();
				this.chessBoradState = (int[][]) ois.readObject();
				this.listChess = (List<Map<String, String>>) ois.readObject();
				this.redUndoNum = (int) ois.readObject();
				this.blackUndoNum = (int) ois.readObject();
				this.isFirstClick = (boolean) ois.readObject();
				this.firstClickChess = (Map<String, String>) ois.readObject();
				this.mapPointerChess = (Map<String, Integer>) ois.readObject();
				this.mapPointerMove = (Map<String, Integer>) ois.readObject();
				this.isGameOver = (boolean) ois.readObject();
				ois.close();
				...
				for (int index = 0; index < this.mapChess.length; index++) {
					int row = Integer.parseInt(this.mapChess[index].get("newRow"));
					int column = Integer.parseInt(this.mapChess[index].get("newColumn"));
					this.labelChess[index].setBounds(this.gridsLeftX + column * this.gridSize - this.chessSize / 2,
							this.gridsTopY + row * this.gridSize - this.chessSize / 2, this.chessSize, this.chessSize);
				}

				// 更新控件状态
				this.jlb_redUndoText.setText("剩" + this.redUndoNum + "次");
				this.jlb_blackUndoText.setText("剩" + this.blackUndoNum + "次");
				this.jlb_redStateText.setText(this.isGameOver ? "已结束" : "进行中");
				this.jlb_blackStateText.setText(this.isGameOver ? "已结束" : "进行中");
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "加载游戏进度时出现错误！", "错误", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void clearSavedGame() {
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "清除保存的游戏进度时出现错误！", "错误", JOptionPane.ERROR_MESSAGE);
		}
	}
